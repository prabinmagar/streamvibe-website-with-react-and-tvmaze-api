...
const EpisodeItem = ({ episodeData }) => {
  return (
    <EpisodeItemWrapper>
      <div className="episode-num text-gray60 text-xl flex items-start font-bold">{episodeData?.number < 10 ? "0" + episodeData?.number : episodeData?.number}</div>
      <div className="episode-full grid">
        <Link to={episodeData?.url} target="_blank" className="episode-img">
          <img src={episodeData?.image?.medium || Images.NoImage} alt="" className="object-fit-cover" />
        </Link>
